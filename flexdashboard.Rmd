...
keywords <- get_keywords(words, keywords_list)
...
inputPanel(
  selectInput(
    inputId = "keyword_type",
    label = "Select Keywords:",
    choices = unique(keywords_list$id),
    multiple = FALSE))
```
...
renderDataTable({
  bigrams <- ngrams_sep |>
      filter(grepl(input$keyword_type, word1) | grepl(input$keyword_type, word2)) |>
