...
-   More Pearl Harbor front pages [here](https://www.nypl.org/blog/2017/12/07/pearl-harbor-front-page).
...
-   Words relating to German people with neutral sentiment (German, Germans) decreased in sentiment, while derogatory terms for German people and German soliders increased.
...
-   Both the Afinn and Bing sentiments were overwhelming negative over the entire time period. This suggests that news that is reported is more often bad than good.
...
```{r}
#Fix the height on this
ngrams_sep <- ngrams |>
  separate(word, into = c("word1", "word2"), sep = " ")

renderDataTable({
  vector <- input$keyword_list
  bigrams <- ngrams_sep |>
      select(as_date, word1, word2) |>
      filter(word1 %in% vector | word2 %in% vector)
  
    bigrams |>
      count(word1, word2, sort = TRUE) |>
      rename(Word1 = word1, Word2 = word2, Count = n) |> # Rename columns
      datatable(
        options = list(
          pageLength = 50,   
          autoWidth = TRUE,
